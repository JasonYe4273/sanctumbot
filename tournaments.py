...
from util import client, tree, send_error, log_command, PT, TEST, ALL
...
async def get_players(interaction: discord.Interaction, tid: int, include_dropped: bool):
    ...
    for ms in message_strs:
        await interaction.channel.send(ms)  # type: ignore[union-attr]
...
async def lfg(interaction: discord.Interaction, tid: int):
    ...
    if start_end[1] and start_end[1] < datetime.now().timestamp():
        await interaction.response.send_message(f"The tournament has already concluded. It ended <t:{start_end[1]}:R>, at <t:{start_end[1]}:F>.", ephemeral=True)
        return
